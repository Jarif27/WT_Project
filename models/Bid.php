...
function insertBid($listing_id, $buyer_id, $amount) {
    ...
    $sql = "INSERT INTO bids (listing_id, buyer_id, amount)
            VALUES (?, ?, ?)";

    $stmt = $conn->prepare($sql);
    return $stmt->execute([$listing_id, $buyer_id, $amount]);
}
...
function getLastTenBids($listing_id) {
    ...
    $sql = "SELECT bids.*, users.name AS bidder_name
            FROM bids
...
function getMyBids($buyer_id) {
    ...
    $sql = "SELECT 
            ...
            FROM bids
            INNER JOIN listings ON bids.listing_id = listings.id
            WHERE bids.buyer_id = ?
...
function getWinnerBid($winner_bid_id) {
    ...
    $sql = "SELECT bids.*, users.name AS winner_name, users.email AS winner_email
            FROM bids
            INNER JOIN users ON bids.buyer_id = users.id
            WHERE bids.id = ?";
